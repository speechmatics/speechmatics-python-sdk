...
class _BaseClient(EventEmitter):
    ...
    def _init_session_info(cls, request_id: Optional[str] = None) -> tuple[SessionInfo, asyncio.Event, asyncio.Event]:
        """
        Create common session state used by all RT clients.
    ...
    def _create_transport_from_config(
        ...
        *,
        ...
        This method centralizes transport creation logic that's shared between
        AsyncClient and AsyncMultiChannelClient, reducing code duplication.
    ...
    async def _recv_loop(self) -> None:
        ...
        The loop:
        1. Waits for incoming messages from the transport
        2. Updates received byte metrics
        3. Emits messages as events based on their "message" field
        """
        try:
            while True:
    ...
    async def close(self) -> None:
        ...
        1. Cancelling the background receive task
        2. Closing the underlying WebSocket transport
        3. Waiting for all cleanup to complete

        Safe to call multiple times - subsequent calls have no effect.
        Always called automatically when using the client as a context manager.
        """
        # Signal closure to prevent new operations
        self._closed_evt.set()

        if self._recv_task and not self._recv_task.done():
            self._recv_task.cancel()
            with contextlib.suppress(Exception):
                # Use a timeout to prevent hanging if recv_task is stuck
